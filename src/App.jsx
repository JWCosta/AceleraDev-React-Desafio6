...
class App extends React.Component {
  constructor(props) {
    ...
    this.organizeByValue = this.organizeByValue.bind(this);
  }
  ...
  searchFilter(name) {
    ...
    if (name.length > 0) {
      const filteredContacts = contacts.filter(contact => {
        return convertToLowercase(contact.name)
          .includes(convertToLowercase(name))
      });
  ...
  render() {
    ...
    return (
      <div data-testid="app" className="app">
        <Topbar />
        <Filters onSearch={this.searchFilter} organize={this.organizeByValue} selectedFilter={sortBy}/>
